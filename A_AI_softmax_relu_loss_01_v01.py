...
"""
#
# A_AI_softmax_relu_loss_01_vnn.py
#
...
# v01 - 1st cut
#
def check_softmax_relu_loss(Y_hot, F2, L=None):
    #
    ...
    try:
        ...
        #
        if len(L.shape) == 1:
            L = L.reshape(1, Y_hot.shape[0])
        #
        myL = np.round(-np.sum(np.transpose(Y_hot) * np.log(F2), axis=0), 4)
        myL = myL.reshape(1, Y_hot.shape[0])
        #
